...
* vpn side of the networking in `/etc/openvpn`
  ...
  * Files `/etc/openvpn/server` `Exit#-ccd` `*` Files in the ccd directory define routes back to the ip ranges of each site eg `iroute 192.168.12.0 255.255.255.224` for ExitA -> Site-A / SSID-A. This is not needed for a road warrior config
